        ...
        <td class="container" bgcolor="#FFFFFF" style="{{ $fontreset }} clear: both !important; display: block !important; max-width: 600px !important; margin: 0 auto; padding: 20px; border: 1px solid #f0f0f0;">
            ...
            <div class="content" style="{{ $fontreset }} display: block; max-width: 600px; margin: 0 auto; padding: 0;">
                <table style="{{ $fontreset }}  width: 100%; {{ $marginreset }}">
                    ...
                    <tr style="{{ $fontreset }} {{ $marginreset }}">
                        <td colspan="3" style="{{ $fontreset }} {{ $marginreset }}">
                            <div style="{{ $fontreset }} {{ $paraeset }}">
                                ...
                                <span style="display:inline-block; padding:3px 5px; color:#fff; background: {{ $ticket->priority->color }}">{{ $ticket->priority->name }}</span>
                                <span style="display:inline-block; padding:3px 5px; color:#fff; background: {{ $ticket->status->color }}">{{ $ticket->status->name }}</span>
                                <span style="display:inline-block; padding:3px 5px; color:#fff; background: {{ $ticket->category->color }}">{{ $ticket->category->name }}</span>
                                {!! $ticket->content !!}
                    ...
                    <tr style="{{ $fontreset }} {{ $marginreset }}">
                        <td colspan="3" style="{{ $fontreset }} {{ $marginreset }}">
                            <p style="{{ $fontreset }} {{ $paraeset }}"><a href="{{ url('/admin') }}">Documentation DesigPond</a></p>
                        </td>
                    </tr>
                </table>
            </div>
